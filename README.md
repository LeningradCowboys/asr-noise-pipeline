...
Full dependencies declaration is in `pyproject.toml` and the actual resolution is in `uv.lock`. You can get all the dependencies by running `pip install -e .`
...
Here the raw files are saved on DagsHub. It can be logged in with your GitHub account, so YOUR_USERNAME is the same as your GitHub username. However, you need to click your avatar on the upper right corner -> Your settings -> Tokens, and there should be a default access token autogenerated on sign up.
...
The data folder originally has 5 subfolders
  data/
    ...
    plots/
    raw/
Raw data are saved on Dagshub. Other four folders are reproducible, which should not be commited as they are fully reproducible. I copy `manifests`, `metrics`, and `plots` to the `results` folder for reviewing convenience. 
